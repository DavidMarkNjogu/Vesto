...
const Checkout = () => {
  ...
  const [formData, setFormData] = useState({
    firstName: '', lastName: '', phone: '', location: '', deliveryNotes: ''
  });
  ...
  const shippingCost = formData.location ? 300 : 0;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      clearCart();
      navigate('/checkout/success', { state: orderPayload });
    } catch (error) {
      console.error(error);
      // Even if online fails, we saved to DB, so redirect to success
      clearCart();
      navigate('/checkout/success', { state: { ...orderPayload, offline: true } });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container mx-auto px-4 max-w-6xl">
        <h1 className="text-3xl font-black text-gray-900 mb-8 text-center sm:text-left">Secure Checkout</h1>
            ...
            <form id="checkout-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input required name="firstName" placeholder="First Name" onChange={handleChange} className="input input-bordered rounded-xl bg-gray-50 focus:bg-white" />
                  <input required name="lastName" placeholder="Last Name" onChange={handleChange} className="input input-bordered rounded-xl bg-gray-50 focus:bg-white" />
                  <input required name="phone" type="tel" placeholder="M-PESA Phone (07...)" onChange={handleChange} className="input input-bordered rounded-xl bg-gray-50 focus:bg-white col-span-2" />
                </div>
              </div>
              ...
              <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100">
                ...
                <div className="space-y-4">
                  <select required name="location" onChange={handleChange} className="select select-bordered w-full rounded-xl bg-gray-50 focus:bg-white">
                    <option value="">Select Region</option>
                    <option value="Nairobi">Nairobi (KES 300)</option>
                    <option value="Nakuru">Nakuru (KES 300)</option>
                    <option value="Mombasa">Mombasa (KES 300)</option>
                  </select>
                  <textarea name="deliveryNotes" placeholder="Delivery Instructions (Optional)" onChange={handleChange} className="textarea textarea-bordered w-full rounded-xl bg-gray-50 focus:bg-white h-24" />
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-gray-900 text-white p-8 rounded-3xl shadow-2xl sticky top-24">
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                <Lock size={18} className="text-green-400"/> Order Summary
              </h3>
              ...
              <div className="space-y-4 mb-8 text-sm text-gray-300">
                ...
                <div className="flex justify-between">
                  <span>Shipping</span>
                  <span>{formData.location ? `KES ${shippingCost}` : '--'}</span>
                </div>
              ...
              <div className="space-y-4">
                <div className="bg-white/10 p-3 rounded-xl flex items-center gap-3 text-xs text-gray-300">
                  <CreditCard className="text-green-400" size={20} />
                  <p>Payment prompt will be sent to <strong>{formData.phone || 'your phone'}</strong>.</p>
                </div>
                ...
                <button 
                  ...
                  form="checkout-form"
                  disabled={loading}
                  className="btn btn-primary w-full rounded-xl text-white font-bold h-12 shadow-lg border-none hover:bg-primary/90"
                >
                  {loading ? <span className="loading loading-spinner"/> : 'PAY NOW'}
                </button>
              </div>
